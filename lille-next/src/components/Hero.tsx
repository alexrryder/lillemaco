...
const AnimatedNotificationBadge = () => {
  ...
  const pulseVariants = {
    animate: {
      boxShadow: [
        "0 0 0 0 rgba(255, 138, 91, 0.7)",
        "0 0 0 10px rgba(255, 138, 91, 0)",
      ],
      transition: {
        duration: 1.5,
        repeat: Infinity,
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="absolute -inset-[1px] rounded-full bg-gradient-to-r from-coral via-lavender to-sage to-coral bg-[length:300%_300%] blur-[0.5px]"
        variants={gradientBorderVariants}
        animate="animate"
      />
      
      {/* Second gradient border for glow effect - removed to make badge smaller */}
      
      {/* Main content */}
      <motion.div 
        className="bg-white px-3 py-1.5 rounded-full border border-transparent shadow-md flex items-center gap-2 relative overflow-hidden z-10"
      ...
      >
        ...
        <motion.div 
          ...
          animate="animate"
          className="relative z-10 flex items-center justify-center bg-gradient-to-br from-coral to-coral/80 rounded-full p-1 shadow-sm"
        >
          <Video className="w-3 h-3 text-white" />
        </motion.div>
        
        <span className="relative z-10 text-xs font-semibold text-black">
          <span className="text-coral font-bold">NEW:</span> Short-form video editing
        </span>
      </motion.div>
    </motion.div>
  );
